...
{#if filteredData[repoIndex].images.length > 0 }
	{#each filteredData[repoIndex].images as tag, tagIndex}
		{#if tag.name === 'latest'}
			<MetadataDrawer tag={tag} repo={filteredData[repoIndex].name} repoIndex={repoIndex} tagIndex={tagIndex} isLatest={true} data={data} />
		{:else}
			<MetadataDrawer tag={tag} repo={filteredData[repoIndex].name} repoIndex={repoIndex} tagIndex={tagIndex} isLatest={false} data={data} />
		{/if}
	{/each}
{:else}
