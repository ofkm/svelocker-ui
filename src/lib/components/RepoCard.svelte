...
<div class="grid grid-cols-1 md:grid-cols-1 gap-4 p-10">
	{#each filteredData as repo, index}
		<CollapsibleCard id={getNamespace(repo.name)} title={getNamespace(repo.name)} description={repo.images.length > 1 ? repo.images.length.toString() + ' Tags Found' : repo.images.length.toString() + ' Tag Found'}>
